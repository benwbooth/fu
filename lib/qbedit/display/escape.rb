...
raise DEC_SPECIAL_CHARS.to_s+','+ALT_DEC_SPECIAL_CHARS.to_s unless
  DEC_SPECIAL_CHARS.length == ALT_DEC_SPECIAL_CHARS.length
...
class KeyqueueTrie
  ...
  def read_mouse_info(keys, more_available)
    ...
    prefix = ''
    prefix += 'shift ' if b & 4 != 0
    prefix += 'meta ' if b & 8 != 0
    prefix += 'ctrl ' if b & 16 != 0
    ...
    if b & 3 == 3
      action = 'release'
      button = 0
    elsif b & MOUSE_RELEASE_FLAG != 0
      action = 'release'
    elsif b & MOUSE_DRAG_FLAG != 0
      action = 'drag'
    else
      action = 'press'
    end
...
def self.process_keyqueue(codes, more_available)
  code = codes[0]
  if code >= 32 && code <= 126
    key = code.chr
...
MOUSE_TRACKING_ON = ESC+"[?1000h"+ESC+"[?1002h"+ESC+"[?1003h"
MOUSE_TRACKING_OFF = ESC+"[?1003l"+ESC+"[?1002l"+ESC+"[?1000l"
