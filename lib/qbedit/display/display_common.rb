...
module DisplayCommon
  ...
  _COLOR_VALUES_256 = [_BASIC_COLOR_VALUES +
      [ _CUBE_STEPS_256.each {|r| 
          _CUBE_STEPS_256.each {|g| 
            _CUBE_STEPS_256.each {|b| [r,g,b] } 
          } 
        }
      ] +
      [_GRAY_STEPS_256.each {|gr| [gr,gr,gr]}]
  ]

  _COLOR_VALUES_88 = [_BASIC_COLOR_VALUES +
      [_CUBE_STEPS_88.each {|r| 
          _CUBE_STEPS_88.each {|g| 
            _CUBE_STEPS_88.each {|b| [r,g,b] }
          }
        }
      ] +
      [_GRAY_STEPS_88.each {|gr| [gr,gr,gr] }]
  ]

  raise unless _COLOR_VALUES_256.length == 256
  raise unless _COLOR_VALUES_88.length == 88
  ...
  def _value_lookup_table(values, size)
      ...
      middle_values = [0] + [
        (0..values.length-1).each {|i| 
          (values[i] + values[i+1] + 1) / 2
        }
      ] + [size]

      lookup_table = []
      (0..middle_values.length-1).each {|i|
  ...
  _CUBE_STEPS_256_16 = _CUBE_STEPS_256.map {|n| int_scale(n, 0x100, 0x10)}
  _GRAY_STEPS_256_101 = _GRAY_STEPS_256.map {|n| int_scale(n, 0x100, 101)}
  _CUBE_STEPS_88_16 = _CUBE_STEPS_88.map {|n| int_scale(n, 0x100, 0x10)}
  _GRAY_STEPS_88_101 = _GRAY_STEPS_88.map {|n| int_scale(n, 0x100, 101)}

  # create lookup tables for 1 hex digit rgb and 0..100 for grayscale values
  _CUBE_256_LOOKUP_16 = (0..15).map {|n| _CUBE_256_LOOKUP(n, 16, 0x100)}
  _GRAY_256_LOOKUP_101 = []
  _CUBE_88_LOOKUP_16 = []
  _GRAY_88_LOOKUP_101 = []

  _CUBE_256_LOOKUP_16 = (0..15).map {|n|
      _CUBE_256_LOOKUP[int_scale(n, 16, 0x100)]
  }
  _GRAY_256_LOOKUP_101 = (0..100).map {|n|
      _GRAY_256_LOOKUP[int_scale(n, 101, 0x100)]
  }
  _CUBE_88_LOOKUP_16 = (0..15).map {|n|
      _CUBE_88_LOOKUP[int_scale(n, 16, 0x100)]
  }
  _GRAY_88_LOOKUP_101 = (0..100).map {|n|
      _GRAY_88_LOOKUP[int_scale(n, 101, 0x100)]
  }
  ...
  def _gray_num_256(gnum=_GRAY_SIZE_256+1)
  ...
  def _gray_num_88(gnum=_GRAY_SIZE_88+1)
      # Return ths color number for gray number gnum.
      # Color cube black and white are returned for 0 and %d respectively
      # since those values aren't included in the gray scale.
  ...
  class AttrSpec
      def initialize(self, fg, bg, colors=256)
          ...
          @_value = 0 | _HIGH_88_COLOR * (colors == 88)
          @foreground = fg
          @background = bg
          if @colors > colors
  ...
  class RealTerminal(object):
      def __init__(self):
          ...
          self._old_signal_keys = nil
          
      def tty_signal_keys(self, intr=nil, quit=nil, start=nil, 
          stop=nil, susp=nil):
          """
          ...
          if intr is not nil: tattr[6][termios.VINTR] = intr
          if quit is not nil: tattr[6][termios.VQUIT] = quit
          if start is not nil: tattr[6][termios.VSTART] = start
          if stop is not nil: tattr[6][termios.VSTOP] = stop
          if susp is not nil: tattr[6][termios.VSUSP] = susp
          
          if intr is not nil or quit is not nil or \
              start is not nil or stop is not nil or \
              susp is not nil:
  ...
  class BaseScreen(object):
      ...
      def register_palette_entry(self, name, foreground, background,
          ...
          if foreground_high is nil:
              foreground_high = foreground
          if background_high is nil:
