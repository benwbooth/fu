...
module CursesDisplay
  ...
  _trans_table = "?"*32+[(32..255).map {|x| x.chr}].join('')
  ...
  class Screen < RealTerminal
    ...
    def _dbg_instr # messy input string (intended for debugging)
      Ncurses.echo()
      @s.nodelay(0)
    ...
    def _dbg_out(str) # messy output function (intended for debugging)
      @s.clrtoeol()
      @s.addstr(str)
      @s.refresh()
      _curs_set(1)
    ...
    def _dbg_query(question) # messy query (intended for debugging)
      _dbg_out(question)
      return _dbg_instr()
    end
